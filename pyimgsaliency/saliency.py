...
def get_saliency_rbd(path):
  ...
  geodesic = np.zeros((len(vertices)+1,len(vertices)+1),dtype=float)
  spatial = np.zeros((len(vertices)+1,len(vertices)+1),dtype=float)
  smoothness = np.zeros((len(vertices)+1,len(vertices)+1),dtype=float)
  adjacency = np.zeros((len(vertices)+1,len(vertices)+1),dtype=float)
  ...
  all_shortest_paths_color = nx.shortest_path(G,method='dijkstra',source=None,target=None,weight='weight')
